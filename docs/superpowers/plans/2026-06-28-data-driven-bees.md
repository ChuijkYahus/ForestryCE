...
- [ ] **Step 3: `BeeSpeciesProvider`** — mirror `MutationProvider`: a static `buildSpecies(RecipeOutput output)` that:
  1. gets `IBeeSpeciesType type = SpeciesUtil.BEE_TYPE.get()`,
  2. creates `ApicultureRegistration reg = new ApicultureRegistration(type)` and calls `DefaultBeeSpecies.register(reg)`,
  3. `reg.forEachSpeciesBuilder((id, builder) -> { ... })`: read fields, run `RecordingGenomeBuilder rec = new RecordingGenomeBuilder(); builder.buildGenome(rec);` to capture `rec.overrides`, assemble a `BeeSpeciesDefinition`, and write JSON.

  For writing JSON through the codec to `src/generated/.../data/forestry/bee_species/<id>.json`: follow exactly how `MutationRecipeBuilder.build(output, id)` emits (it wraps a `MutationRecipe` and calls `output.accept(...)`). Bee species are **not** recipes, so do not use `RecipeOutput.accept`. Instead use the `PackOutput`/`DataProvider.saveStable(...)` path with `BeeSpeciesDefinition.codec()` → `JsonElement`, writing to `PackOutput.Target.DATA_PACK` under `forestry:bee_species/<path>`. If `Data.java` uses ModKit's `DataHelper`, check whether it exposes a generic JSON sink; otherwise register `BeeSpeciesProvider` as a vanilla `DataProvider` via `generator.addProvider(event.includeServer(), new BeeSpeciesProvider(packOutput, lookupProvider))` in `Data.gatherData` (alongside the existing `generator.addProvider(...)` calls).
...
- [ ] **Step 2: `BeeSpeciesManager`** extends `SimpleJsonResourceReloadListener` over folder `"bee_species"` with the standard JSON gson. Singleton (`INSTANCE`) holding the last-parsed `Map<ResourceLocation, BeeSpeciesDefinition>` (`volatile`, default empty) accessible via `getDefinitions()`. In `apply(Map<ResourceLocation, JsonElement> object, ResourceManager rm, ProfilerFiller profiler)`: decode each entry via `BeeSpeciesDefinition.codec()` (log-and-skip on `DataResult` error), store the map, then call `GeneticsReloadHandler.rebuildSpecies(map)`.
