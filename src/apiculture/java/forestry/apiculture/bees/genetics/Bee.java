...
import forestry.core.engine.genetics.IndividualLiving;
...
/*
	...
	public List<ItemStack> produceStacks(IBeeHousing housing) {
		...
		float speed = beeHousingModifier.modifyProductionSpeed(this.genome, this.genome.getActiveValue(BeeChromosomes.SPEED))/* * beeModeModifier.modifyProductionSpeed(genome, 1f)*/;
	...
	public IBee spawnPrincess(IBeeHousing housing) {
		...
		return createOffspring(housing, this.mate, getGeneration() + 1);
	}
	...
	public List<IBee> spawnDrones(IBeeHousing housing) {
		...
		for (int i = 0; i < toCreate; i++) {
			IBee offspring = createOffspring(housing, this.mate, 0);
	...
	private static ImmutableList<AllelePair<?>> mutateSpecies(IBeeHousing housing, IGenome parent1, IGenome parent2) {
		return SpeciesUtil.<IBeeSpecies>mutateSpecies(housing.getLevel(), housing.getBlockPos(), housing.getOwner(), parent1, parent2, BeeChromosomes.SPECIES, (mutation, level, pos, firstGenome, secondGenome, climate) -> {
