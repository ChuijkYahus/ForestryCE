package forestry.agriculture.planter.gui;
...
import forestry.agriculture.planter.inventory.InventoryPlanter;
...
public class ContainerPlanter extends ContainerLiquidTanks<TilePlanter> {
	...
	public ContainerPlanter(int windowId, Inventory playerInventory, TilePlanter tileForestry) {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				addSlot(new SlotFiltered(this.tile.getInternalInventory(), InventoryPlanter.CONFIG.resourcesStart + j + i * 2, 11 + j * 18, 65 + i * 18));
			}
		}
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				addSlot(new SlotFiltered(this.tile.getInternalInventory(), InventoryPlanter.CONFIG.germlingsStart + j + i * 2, 71 + j * 18, 65 + i * 18));
			}
		}
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				addSlot(new SlotOutput(this.tile.getInternalInventory(), InventoryPlanter.CONFIG.productionStart + j + i * 2, 131 + j * 18, 65 + i * 18));
			}
		}

		// Fertilizer
		addSlot(new SlotFiltered(this.tile.getInternalInventory(), InventoryPlanter.CONFIG.fertilizerStart, 83, 22));
		// Can Slot
		addSlot(new SlotLiquidIn(this.tile.getInternalInventory(), InventoryPlanter.CONFIG.canStart, 178, 18));
	}
