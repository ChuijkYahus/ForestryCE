...
public final class GeneticRegistration implements IGeneticRegistration {
	...
	public GeneticRegistration() {
		...
		// each genus's default chromosomes) now ships as datapack JSON, generated by TaxonProvider and merged into the
		// live taxonomy on datapack (re)load. Only taxa contributed in code by add-ons/KubeJS (via defineTaxon) land in
		// baseTaxa now; the datapack merges on top of that.
	}

	/**
	 * Registers a root {@link TaxonomicRank#DOMAIN} taxon (one with no parent). Used by the data generator that authors
	 * base Forestry's taxonomy; the public {@link #defineTaxon} entrypoints only add taxa under an existing parent.
	 */
	public ITaxonBuilder defineDomain(String name) {
		return registerTaxon(TaxonomicRank.DOMAIN, name);
	}
