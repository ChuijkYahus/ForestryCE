 ...
 * Forge fluid handler capability. A vanilla glass bottle has no such capability, so
 ...
 * <p>
 * Water previously mapped to {@link Items#HONEY_BOTTLE}, so a tank of water produced honey. The water test below is
 ...
 * only reaches it once every 20 ticks and only after {@code TilePowered} has satisfied its energy and recipe
 * requirements, none of which this table depends on.
 */
...
public class SqueezerBottlingTest {
	...
	/**
	 * The regression: a tank of water must bottle into a water bottle, not a honey bottle.
	 */
	...
	public static void waterBottlesIntoAWaterBottle(GameTestHelper helper) {
		...
		helper.assertTrue(output.is(Items.POTION),
			"squeezing water into a glass bottle produced " + output.getItem() + " instead of a water bottle");
		Potion potion = PotionUtils.getPotion(output);
		helper.assertTrue(potion == Potions.WATER,
			"expected a plain water bottle, found the potion " + potion);
