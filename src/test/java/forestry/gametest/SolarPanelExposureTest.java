 ...
 * <p>
 * The rig is built at absolute coordinates in open air rather than inside the test plot. GameTest buries its plots at
 * roughly y=-60, and in this world that is under a warm ocean: there is no sky access down there, and a shaft dug up
 * to the surface simply refills with water, which blocks sky light. The plot's own chunk is clear air well above sea
 * level, so the rig goes there and is torn down again at the end.
 */
